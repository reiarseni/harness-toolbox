...
|---|---|---|
| `Documentacion/…` (`--section`, default per language; or `wiki.section` in `_meta/documake.json`) | CI only | rewritten each merge; stale generated pages removed; each page carries `<!-- documake:generated … -->` after its front matter, and a one-line footer linking the source file |
...
**How pages are named, and why it matters.** GitLab builds a wiki page's title
...
- puts the real title, accents and all, in `title:` front matter, and drops the
  now-duplicated `# H1` from the body (GitLab already prints the title);
- rewrites internal links to wiki paths **without** `.md`, keeping anchors;
- emits a collapsible `_sidebar` (`--sidebar-flat` for a plain list if an old
  GitLab sanitises `<details>`), with the group prefix stripped from labels;
- adds `[[_TOC_]]` to long pages and a prev/index/next footer;
- deletes the pages of the previous section names (`documentacion-del-proyecto`,
  `project-documentation`) on the first run after an upgrade.

**Renaming the section breaks its URLs** — a wiki has no redirects. Say so
before the first publish of an existing wiki, and afterwards update the link in
the wiki home.
...
the publisher reaches the wiki without touching the docs. `scripts/test_publish.py`
covers renaming, link rewriting, front matter, the sidebar and the migration:
run `python3 -m unittest discover -s scripts` after touching `publish.py`.
